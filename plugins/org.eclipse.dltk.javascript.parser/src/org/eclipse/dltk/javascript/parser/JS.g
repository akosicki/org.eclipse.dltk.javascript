...
arrayLiteral
	: lb=LBRACK ( arrayItem ( COMMA arrayItem? )* )? rb=RBRACK
	-> ^( ARRAY[$lb, "ARRAY"] arrayItem* $rb)
	;
...
The EOL situation is handled by promoting the EOL (or MultiLineComment with an EOL) present to on channel 
